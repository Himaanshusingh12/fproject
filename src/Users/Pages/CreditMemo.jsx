...
function CreditMemo() {
	...
	//section for fetch sales invoice according to selected customer
	const [salesInvoices, setSalesInvoices] = useState([]);
	const fetchSalesInvoices = async (customerId) => {
		try {
			const response = await axios.get(`${BACKEND_URL}/api/sales-invoices/by-customer/${customerId}`);
			console.log("Fetched Invoices No:", response.data);
	...
	const handleCustomerChange = (e) => {
		...
		if (selectedCustomer) {
			setFormData((prevData) => ({
				...
				email: selectedCustomer.email_sales,
				currency: selectedCustomer.currency,
				shipping_address: selectedCustomer.address,
			}));
		} else {
			// Reset to default values if no customer is found
			setFormData((prevData) => ({
				...prevData,
				customer_address: "",
				email: "",
	...
	const fetchCreditmemoNumber = async () => {
		try {
			const response = await axios.get(`${BACKEND_URL}/api/next_salescreditmemo_number`);
			console.log("Fetched creditmemo No:", response.data);
			setFormData({
				...formData,
				creditmemo_no: response.data.next_creditmemo_no,
			});
		} catch (error) {
	...
	const handleCreditmemoNumberChange = async (e) => {
		const newCreditmemoNumber = e.target.value;

		// Update the formData with the new invoice number
		setFormData({
			...formData,
			creditmemo_no: newCreditmemoNumber,
		});

		// Check if the entered invoice number already exists in the database
		try {
			const response = await axios.get(`${BACKEND_URL}/api/check-creditmemo?creditmemoNumber=${newCreditmemoNumber}`);

			if (response.data.exists) {
				setError("This creditmemo number is already in use.");
			} else {
				setError("");
			}
		} catch (error) {
			console.error("Error checking creditmemo number:", error);
		}
	};
	...
	const handleChange = (e) => {
		...
		if (name === "posting_date" || name === "payment_terms") {
			const newDueDate = calculateDueDate(formData.posting_date, formData.payment_terms);
			setFormData((prevData) => ({
				...prevData,
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await axios.post(`${BACKEND_URL}/api/sales-creditmemo`, formDataToSend, {
				headers: {
			...
			if (response.status === 201) {
				...
				toast.error(`Failed to Create Sales Credit memo : ${response.statusText}`);
			}
		} catch (error) {
			console.error("Error Creating Sales credit memo", error);
			toast.error(`Failed to Create Sales credit memo: ${error.message}`);
		}
	};
	return (
		<>
								...
								<div className="col-md-12">
									<div className="card card-primary">
										<div className="card-header d-flex justify-content-between align-items-center">
											...
											<Link to="/view-creditmemo" className="btn btn-success ml-auto">
												View All Sales Creditmemo
											</Link>
										</div>
											...
											<div className="card-body">
												<div className="row">
													...
													<div className="col-md-6">
														...
														<div className="col-md-10">
															<div className="form-group">
																<label htmlFor="credit">Credit Against Sales Invoice</label>
																<select
																	className="form-control"
																	id="credit"
																	name="credit"
													...
													<div className="col-md-6">
														<div className="form-group">
															<label htmlFor="creditmemo_no">Sales Credit Memo No.</label>
															<input
																type="text"
																name="creditmemo_no"
																id="creditmemo_no"
																className="form-control col-md-8"
																value={formData.creditmemo_no}
