...
function DebitMemo() {
	...
	//section for fetch Purchase invoice according to selected vendor
	const [purchaseInvoices, setPurchaseInvoices] = useState([]);
	// Fetch purchase invoices when a vendor is selected
	const fetchPurchaseInvoices = async (vendorId) => {
		try {
			const response = await axios.get(`${BACKEND_URL}/api/purchase-invoices/by-vendor/${vendorId}`);
			console.log("Fetched Purchase Invoices No:", response.data);
	...
	const handleVendorChange = (e) => {
		...
		if (selectedVendor) {
			setFormData((prevData) => ({
				...
				email: selectedVendor.email_payment,
				currency: selectedVendor.currency,
				shipping_address: selectedVendor.address_send,
			}));
		} else {
			setFormData((prevData) => ({
				...prevData,
	...
	useEffect(() => {
		...
		getTaxes();
		getPaymentterm();
		fetchDebitmemoNumber();
		fetchPurchaseInvoices();
	}, []);

	const fetchDebitmemoNumber = async () => {
		try {
			const response = await axios.get(`${BACKEND_URL}/api/next_purchasedebitmemo_number`);
			// console.log("Fetched Debitmemo No:", response.data);
			setFormData({
				...formData,
				debitmemo_no: response.data.next_debitmemo_no,
			});
		} catch (error) {
	...
	const handleDebitmemoNumberChange = async (e) => {
		const newDebitmemoNumber = e.target.value;

		// Update the formData with the new invoice number
		setFormData({
			...formData,
			debitmemo_no: newDebitmemoNumber,
		});

		// Check if the entered invoice number already exists in the database
		try {
			const response = await axios.get(`${BACKEND_URL}/api/check-debitmemo?debitmemoNumber=${newDebitmemoNumber}`);

			if (response.data.exists) {
				setError("This Debitmemo number is already in use.");
			} else {
				setError("");
			}
		} catch (error) {
			console.error("Error checking debitmemo number:", error);
		}
	};
	...
	// function for fetch vendor name from backend
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await axios.post(`${BACKEND_URL}/api/purchase-debitmemo`, formDataToSend, {
				headers: {
			...
			if (response.status === 201) {
				...
				toast.error(`Failed to Create Purchase Debit memo : ${response.statusText}`);
			}
		} catch (error) {
			console.error("Error Creating Purchase Debit memo", error);
			toast.error(`Failed to Create Purchase Debit memo: ${error.message}`);
		}
	};
	return (
		<>
									...
									<div className="card card-primary">
										...
										<form onSubmit={handleSubmit}>
											<div className="card-body">
												<div className="row">
													<h5 className="section-title">Debit details</h5>
													<div className="col-md-6">
														...
														<div className="col-md-10">
															<div className="form-group">
																<label htmlFor="vendor_address">Bill To Address</label>
																<textarea
														...
														<div className="col-md-10">
															<div className="form-group">
																<label htmlFor="debit">Debit Against Purchase Invoice</label>
																<select
																	className="form-control"
																	id="debit"
																	name="debit"
													...
													<div className="col-md-6">
														<div className="form-group">
															<label htmlFor="debitmemo_no">Purchase Debit Memo No.</label>
															<input
																type="text"
																name="debitmemo_no"
																id="debitmemo_no"
																className="form-control col-md-8"
																value={formData.debitmemo_no}
																onChange={handleDebitmemoNumberChange}
															/>
