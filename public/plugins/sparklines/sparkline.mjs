export default class Sparkline {
	...
	draw(points = []) {
		...
		const bottomValue =
			this.options.minValue != undefined ? this.options.minValue : Math.min(minValue, this.options.maxMinValue != undefined ? this.options.maxMinValue : minValue);
		const topValue =
			this.options.maxValue != undefined ? this.options.maxValue : Math.max(maxValue, this.options.minMaxValue != undefined ? this.options.minMaxValue : maxValue);
		let minX = offsetX;
		let maxX = offsetX;

		let x = offsetX;
		const y = (index) => (topValue === bottomValue ? offsetY + height / 2 : offsetY + height - ((points[index] - bottomValue) / (topValue - bottomValue)) * height);
