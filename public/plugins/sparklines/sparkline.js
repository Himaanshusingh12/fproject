...
//   function getY(points, minValue, maxValue, offsetY, height, index) {
//     var range = maxValue - minValue;
//     if (range == 0) {
//       return offsetY + height / 2;
//     } else {
//       return (offsetY + height) - ((points[index] - minValue) / range) * height;
//     }
//   }
...
//     var x = offsetX;
//     var y = getY.bindn(null, points, bottomValue, topValue, offsetY, height);
...
(function (root, factory) {
	...
	Sparkline.prototype.draw = function (points) {
		...
		var minValue = Math.min.apply(Math, points);
		var maxValue = Math.max.apply(Math, points);
		var bottomValue = this.options.minValue != null ? this.options.minValue : Math.min(minValue, this.options.maxMinValue != null ? this.options.maxMinValue : minValue);
		var topValue = this.options.maxValue != null ? this.options.maxValue : Math.max(maxValue, this.options.minMaxValue != null ? this.options.minMaxValue : maxValue);
		var minX = offsetX;
		var maxX = offsetX;

		var x = offsetX;
		var y = getY.bind(null, points, bottomValue, topValue, offsetY, height); // Passing `points` explicitly
		var delta = width / (points.length - 1);
